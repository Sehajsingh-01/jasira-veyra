    ...
    <main className="min-h-screen bg-midnight text-cream py-24 px-6 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <header className="text-center mb-16 space-y-6">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-warm-gold/10 border border-warm-gold/30 text-warm-gold font-ui text-xs tracking-[0.3em] uppercase">
            <Sparkles size={12} className="animate-pulse" />
            <span>The Bloomverse Novel Series</span>
            <Sparkles size={12} className="animate-pulse" />
          </div>

          <Link href="/" title="Return to Main Website" className="inline-block group cursor-pointer">
            <h1 className="font-display text-5xl sm:text-6xl md:text-7xl lg:text-8xl tracking-wide group-hover:text-warm-gold group-hover:drop-shadow-[0_0_25px_rgba(212,168,83,0.5)] transition-all duration-300">
              Jas of Duskbloom
            </h1>
          </Link>

          <p className="font-handwritten text-2xl sm:text-3xl text-lavender/80 max-w-2xl mx-auto">
            "A story about the quiet power that saves what shouting never could."
          </p>

          <p className="font-body text-cream/75 max-w-2xl mx-auto leading-relaxed text-sm sm:text-base pt-2">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 pt-4 text-xs font-ui text-lavender/70">
            ...
            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/10">
              <Layers size={14} className="text-warm-gold" />
          ...
          <div className="pt-6 flex flex-col items-center justify-center gap-2">
            ...
            <p className="font-ui text-[11px] text-warm-gold/70 tracking-wider">
...
function ChapterCardContent({ chapter }: { chapter: any }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="relative h-56 w-full bg-midnight/70 overflow-hidden">
        <Image
          src={cover}
          alt={chapter.title}
          fill
        ...
        <div className="absolute top-4 left-4">
          <span className="font-ui text-[10px] tracking-widest text-warm-gold bg-midnight/80 border border-warm-gold/30 px-3 py-1 rounded-full uppercase">
            Chapter {chapter.number.toString().padStart(2, '0')}
          </span>
        </div>

        <div className="absolute bottom-3 right-4 font-ui text-[11px] text-cream/70 bg-black/50 px-2.5 py-1 rounded backdrop-blur-sm">
      ...
      <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
        <div>
          <h2 className="font-display text-2xl sm:text-3xl text-cream group-hover:text-warm-gold transition-colors mb-2">
            {chapter.title}
          </h2>
          {chapter.subtitle && (
            <p className="font-handwritten text-lg text-lavender/70 mb-3">
              {chapter.subtitle}
            </p>
          )}
        ...
        <div className="pt-4 border-t border-white/5 flex items-center justify-between font-ui text-xs text-warm-gold/80 group-hover:text-warm-gold">
